...
class House:
    ...
    def log_house_info(self):
        self.log(massage=f'В доме: денег {self.money}, человеческой еды {self.food},'
                         f'кошачьей еды {self.cat_food}, грязи {self.dirt}', color='yellow')

    def lost_food(self):
        self.food = 0
        self.log('Пропала вcя еда из холодильника!', color='red')
...
class Man(Person):
    ...
    def log_man_info(self):
        self.log(f'{self.name}: сытость {self.fullness}, счастье {self.happiness}', color='cyan')

    def eat(self):
        if self.house.food > 0:
            if self.house.food >= self.MAN_EATING_POINTS:
                eating_points = self.MAN_EATING_POINTS
                self.log(f'{self.name} хорошо поел(а)')
            else:
                eating_points = self.house.food
                self.log(f'{self.name} немного поел(а)')
            ...
            self.fullness -= 10
            self.log(f'{self.name} нет еды!', color='red')
    ...
    def buy_cat_food(self):
        ...
        if self.house.money >= self.CAT_EATING_POINTS_FOR_BUY:
            ...
            self.log(f'{self.name} купил(а) коту еды')
        else:
            self.log(f'{self.name} нет денег на еду для кота!', color='red')

    def stroking_cat(self):
        self.fullness -= 10
        self.happiness += 5
        self.totals['stroking_cat_times'][1] += 1
        self.log(f'{self.name} гладил(а) кота')

    def act(self):
        if self.is_dead:
            self.log(f'{self.name} умер(ла)!', color='red')
            return True
        if self.is_hungry:
            self.eat()
            return True
...
class Husband(Man):
    ...
    def work(self):
        ...
        self.log(f'{self.name} сходил на работу')
    ...
    def gaming(self):
        ...
        self.log(f'{self.name} играл в Танки')
...
class Wife(Man):
    ...
    def shopping(self):
        ...
        if self.house.money >= self.FOOD_LEVEL_FOR_LIFE:
            ...
            self.log(f'{self.name} купила еды')
        else:
            self.log(f'{self.name} нет денег на еду!', color='red')
    ...
    def clean_house(self):
        ...
        self.log(f'{self.name} убралась в доме')
    ...
    def watch_series(self):
        ...
        if series_mood == 1:
            ...
            self.log(f'{self.name} смотрела сериал, осталась довольна')
        else:
            self.happiness -= 5
            self.totals['happiness'][1] -= 5
            self.log(f'{self.name} смотрела сериал, расстроилась')
    ...
    def buy_fur_coat(self):
        ...
        if self.house.money >= 350:
            ...
            self.log(f'{self.name} купила шубу')
        else:
            self.log(f'{self.name} нет денег на шубу!', color='red')
...
class Child(Man):
    ...
    def sleep(self):
        ...
        self.log(f'{self.name} спал весь день')
...
class Cat(Person):
    ...
    def soil(self):
        ...
        self.log(f'{self.name} драл обои')

    def act(self):
        if self.is_dead:
            self.log(f'{self.name} умер!', color='red')
            return
        dice = randint(1, 6)
        if self.is_hungry:
            self.eat()
...
def emulate_life(num_cats, need_logging=False):
    ...
    cats = [
        Cat(name=choice(cat_names), house=home, need_logging=need_logging)
